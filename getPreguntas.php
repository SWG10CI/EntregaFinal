<?php
			...
			echo'<fieldset style="border: 1px solid black"><legend style="border: 1px solid black ;margin:15px;padding: 0.5%">Preguntas</legend>';
			echo '<table border=1> <tr> <th> Enunciado </th><th> Complejidad </th> <th> Tema </th></tr>';

			while ($pregunta=mysqli_fetch_array($preguntas)){
				echo '<tr>';
				echo '<td>' . $pregunta["enunciado"] . '</td>' ;
				echo '<td>' . $pregunta["complejidad"] . '</td>' ;
				echo '<td>' . $pregunta["tema"] . '</td>' ;
				echo '</tr>';
			}
			echo '</fieldset>';
